...
contract VertexRouter is ISwapRouter {
    ...
    {
        require(path.length == 2, "VertexRouter: only supports 2-token path");

        address tokenIn = path[0];
        address tokenOut = path[1];

        uint256 rate = exchangeRate[tokenIn][tokenOut];
        require(rate > 0, "VertexRouter: no exchange rate found");

        uint256 amountOut = rate * amountIn / RATE_DECIMAL;
        require(amountOut >= minAmountOut, "VertexRouter: slippage too high");

        bool inputSuccess = IERC20(tokenIn).transferFrom(msg.sender, address(this), amountIn);
        require(inputSuccess, "VertexRouter: failed to pull token from adapter");

        bool outputSuccess = IERC20(tokenOut).transfer(to, amountOut);
        require(outputSuccess, "VertexRouter: failed to transfer token to user");
