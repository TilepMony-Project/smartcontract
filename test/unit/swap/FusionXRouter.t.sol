...
contract FusionXRouterTest is Test {
  ...
  function test_SuccesfulSwap_RouterLogic() public {
    ...
    assertEq(tokenB.balanceOf(RECEIVER), receiverTokenBBalanceBefore + EXPECTED_OUT, "Adapter TokenA balance incorrect");
    assertEq(tokenB.balanceOf(address(router)), routerTokenBBalanceBefore - EXPECTED_OUT, "Router TokenB balance incorrect");
  }
  ...
  function test_RevertWhen_NoRateSet() public {
    ...
    vm.expectRevert("FusionXRouter: no exchange rate found");
    router.swapExactTokensForTokens(
      AMOUNT_IN,
      MIN_AMOUNT_OUT,
      path,
