 ...
 */
contract VertexAdapter is ISwapAdapter {
    /**
     * @notice Immutable reference to the `ISwapRouter` contract that performs the actual exchange.
     * @dev This is the address of the `VertexRouter` instance that this adapter utilizes.
     */
    ISwapRouter public immutable ROUTER;

    /**
     * @notice Constructor for the VertexAdapter.
        ...
        returns (uint256 amountOut)
    {
        ...
        require(pullSuccess, "VertexAdapter: failed to pull token from aggregator to adapter");

        bool approveSuccess = IERC20(tokenIn).approve(address(ROUTER), amountIn);
        require(approveSuccess, "VertexAdapter: failed to approve router to spend the tokens");
