...
contract VertexAdapterTest is Test {
  ...
  function test_SuccesfulSwap_AdapterLogic() public {
    ...
    assertEq(tokenA.balanceOf(AGGREGATOR), aggregatorTokenABalanceBefore - AMOUNT_IN, "Aggregator TokenA balance incorrect");
    assertEq(tokenA.balanceOf(address(adapter)), adapterTokenABalanceBefore, "Adapter TokenA balance should be 0");
    assertEq(tokenB.balanceOf(RECEIVER), receiverTokenBBalanceBefore + EXPECTED_OUT, "Receiver TokenB balance incorrect");
  }
