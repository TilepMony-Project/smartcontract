 ...
 */
contract MerchantMoeRouter is ISwapRouter, Ownable {
    ...
    {
        require(path.length == 2, "MerchantMoeRouter: only supports 2-token path");

        address tokenIn = path[0];
        address tokenOut = path[1];

        uint256 rate = exchangeRate[tokenIn][tokenOut];
        require(rate > 0, "MerchantMoeRouter: no exchange rate found");
